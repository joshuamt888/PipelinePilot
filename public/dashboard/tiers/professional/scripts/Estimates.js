...
window.EstimatesModule = {
    ...
    estimates_openDetailView(estimateId) {
        ...
        overlay.innerHTML = `
            ...
            <div class="estimate-detail-modal">
                ...
                <div class="estimate-detail-actions">
                    <button class="estimate-action-btn" style="background: #6366f1; border: 1px solid #6366f1; color: white;" data-action="download-pdf" data-id="${estimate.id}">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                            <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4m4-5l5 5 5-5m-5 5V3" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
        ...
        setTimeout(() => {
            ...
            // Download PDF action
            const downloadBtn = overlay.querySelector('[data-action="download-pdf"]');
            if (downloadBtn) {
                downloadBtn.addEventListener('click', () => {
                    this.estimates_downloadClientCopy(estimate);
                });
            }
        }, 0);
    },
    ...
    /**
     * Download professional PDF client copy of estimate
     */
    estimates_downloadClientCopy(estimate) {
        try {
            ...
            doc.text('ESTIMATE', 15, 25);

            doc.setFontSize(12);
            doc.setFont(undefined, 'normal');
            doc.text(estimate.estimate_number || 'EST-???', 15, 33);

            // Company info (right side of header)
            doc.setFontSize(10);
            doc.setFont(undefined, 'normal');
            doc.text('SteadyManager', 140, 20);
            const dateStr = new Date().toLocaleDateString('en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric'
            });
            doc.text(dateStr, 140, 26);
            ...
            doc.setFontSize(18);
            doc.setFont(undefined, 'bold');
            doc.text(estimate.title || 'Untitled', 15, yPos);
            yPos += 10;
            ...
            if (lead) {
                ...
                doc.setFont(undefined, 'normal');
                doc.setTextColor(...darkGray);
                doc.text(lead.name, 15, yPos);

                if (lead.company) {
                    yPos += 5;
                    doc.text(lead.company, 15, yPos);
                }

                if (lead.email) {
                    yPos += 5;
                    doc.text(lead.email, 15, yPos);
                }

                if (lead.phone) {
                    yPos += 5;
                    doc.text(lead.phone, 15, yPos);
                }
            ...
            if (lineItems.length > 0) {
                ...
                doc.setFontSize(14);
                doc.text(`$${(estimate.total_price || 0).toFixed(2)}`, 145, yPos + 12);
            ...
            if (estimate.expires_at) {
                ...
                doc.setTextColor(...lightGray);
                const expiryDate = new Date(estimate.expires_at).toLocaleDateString('en-US', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                });
                doc.text(`This estimate expires on ${expiryDate}`, 15, yPos);
            }
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                doc.text('Generated by SteadyManager', 15, 287);
            }

            // Download the PDF
            const fileName = `${estimate.estimate_number || 'estimate'}_${estimate.title.replace(/[^a-z0-9]/gi, '_')}.pdf`;
            doc.save(fileName);
            ...
            window.SteadyUtils.showToast('Failed to generate PDF. Please try again.', 'error');
        }
    },
